...
public class Productos_SQL {
    ...
    public Ajustes buscarDatos(){
        ...
        try {
            ...
            rs = pst.executeQuery();
            if (rs.next()) {
                ajustes.setId_acuarela(rs.getInt("id_acuarela"));
                ajustes.setRuc_acuarela(rs.getString("ruc_acuarela"));
                ajustes.setNom_acuarela(rs.getString("nom_acuarela"));
                ajustes.setTelf_acuarela(rs.getString("telf_acuarela"));
                ajustes.setDirec_acuarela(rs.getString("direc_acuarela"));
                ajustes.setMensaje_acuarela(rs.getString("mensaje_acuarela"));
            }
        } catch (Exception e) {
            System.out.println(e.toString());
        }
    ...
    public boolean modificarDatos(Ajustes ajustes){
        String SQL = "UPDATE ajustes SET ruc_acuarela=?, nom_acuarela=?, telf_acuarela=?, direc_acuarela=?, mensaje_acuarela=? WHERE id_acuarela=?";
        try {
            pst = con.prepareStatement(SQL);
            pst.setString(1, ajustes.getRuc_acuarela());
            pst.setString(2, ajustes.getNom_acuarela());
            pst.setString(3, ajustes.getTelf_acuarela());
            pst.setString(4, ajustes.getDirec_acuarela());
            pst.setString(5, ajustes.getMensaje_acuarela());
            pst.setInt(6, ajustes.getId_acuarela());
            pst.execute();
            return true;
        } catch (Exception e) {
            System.out.println(e.toString());
